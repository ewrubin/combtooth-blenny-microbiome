...
Fig4A = ggplot(data.long.taxon.metadata, aes(x = sample, y = Genus, fill = Class)) + 
...
ggsave("Figure-4A-top-genera-SDA-8x6.tiff", units="in", width=8, height=6, dpi=300, compression = 'lzw')
ggsave("Figure-4A-top-genera-SDA-8x6.pdf", units="in", width=8, height=6, dpi=300)
...
Fig4B = ggplot(data.long.taxon.metadata, aes(x = sample, y = Genus, fill = Class)) + 
  geom_point(aes(size = proportion), alpha = 0.75, shape = 21) + 
...
ggsave("Figure-4B-top-genera-notDA-8x5.tiff", units="in", width=8, height=5, dpi=300, compression = 'lzw')
ggsave("Figure-4B-top-genera-notDA-8x5.pdf", units="in", width=8, height=5, dpi=300)
...
Fig4_final <- ggdraw(Fig4_combined) +
  draw_label(
    "A. SDA top genera",
    x = 0.02, y = 0.985,
    hjust = 0,
